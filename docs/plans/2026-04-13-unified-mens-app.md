...
- 2026-04-14: **Testing port (read-only) complete.** `/[playerId]/testing` route shows Iker's physical test sessions (body composition, per-category results with tier/delta/sparkline, collapsible session history). Gated to in-program players; alumni redirect to info. Closes the last feature-parity gap with the Vite app — players migrating to the unified portal no longer lose access to their numbers. Test entry stays in Staff App; this is a view only.
- 2026-04-14: **Scout feedback loop closed.** `notifyScout` now also fires on `withdrawn` transitions and on `scheduled` transitions made via the prospect detail page dropdown (previously only the approve-modal path notified). Scouts now learn of every meaningful trial outcome automatically: scheduled, accepted, rejected, withdrawn.
